...
class SdoOrchestrator:
        ...
        if configuration.PRIVATE_UTILITY == "SERVICE" or service_specific:
            ...
            if self.sdo_name == 'sdo0':
                if self._DEBUG_first is True:
                    return self._cdn_marginal_utility(bid_bundle, service, function, node, submodular=spu)
                else:
                    return self._traffic_based_marginal_utility(bid_bundle, service, function, node, submodular=spu)
    ...
    def _cdn_marginal_utility(self, bid_bundle, service, function, node, submodular=True):
        """
        ...
        # take misses statistics from file
        stat_filename = "config/use_case_stat/cache_statistics.json"
        with open(stat_filename, "r") as stat_file:
            stats = json.loads(stat_file.read())
        max_storage = stats["max-caches"]
        misses_ratio = stats["misses-ratio"][node]
        current_allocated_storage = stats["current-caches-storage"][node]
        already_scheduled = sum([self.rap.consumption[bid_bundle[s]['function']]['storage']
                                 for s in bid_bundle if bid_bundle[s]['node'] == node])
        function_storage = self.rap.consumption[function]['storage']

        if misses_ratio == 0:
            # no traffic on this node
            if already_scheduled > 0:
                if function == "f11":
                    return 2
                else:
                    return 0
            elif function_storage > 0:
                if function == "f9":
                    return 3
                else:
                    return 5
        ...
        if misses_ratio <= 0.07:
            # caches ok on that node
            if already_scheduled >= current_allocated_storage:
                if function == "f11":
                    return 2
                else:
                    return 0
            elif function_storage > 0:
                # return (current_allocated_storage - already_scheduled)/function_storage
                if already_scheduled == 0:
                    if function == "f9":
                        return 6
                    else:
                        return 7
                if current_allocated_storage - already_scheduled > function_storage:
                    if function == "f9":
                        return 4
                    else:
                        return 5
                ...
                return 1
        elif current_allocated_storage == 0:
            # no caches on that node (so ratio is 1 or 0)
            if misses_ratio == 1:
                if already_scheduled >= max_storage / len(self.rap.nodes):
                    if function == "f11":
                        return 2
                    else:
                        return 0
                elif already_scheduled == 0:
                    if function_storage > 0:
                        if function == "f9":
                            return 50
                        else:
                            return 100
            ...
            estimated_needed = current_allocated_storage/(1 - misses_ratio)
            if already_scheduled >= estimated_needed:
                if function == "f11":
                    return 1
                else:
                    return 0
                ...
                return 3
            elif estimated_needed - already_scheduled > function_storage:
                utility = max(((estimated_needed - already_scheduled) / estimated_needed) * 100, 7)
                # utility = ((estimated_needed - already_scheduled) / estimated_needed)
                # utility = (100 - 6)*utility + 6
                #utility -= (10 - (function_storage/100))
                return utility
            else:
                utility = max(((estimated_needed - already_scheduled) / estimated_needed) * 100, 7)
                # utility = ((estimated_needed - already_scheduled) / estimated_needed)
